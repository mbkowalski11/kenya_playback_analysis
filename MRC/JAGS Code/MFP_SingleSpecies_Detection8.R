...
# SINGLE-SPECIES VISITATION WITH SPATIAL GAUSSIAN PROCESS ON LAMBDA
...
det.out.wZT8.ele <- jags(data = data, inits = inits, parameters.to.save = parameters, 
...
save(list = c("det.out.wZT8.ele"), file = 'MFP_DetRate_Model8ele.rda')

###################################################################################################
###################################################################################################
# Posterior Sampling of Bayesian P-Values and Percent Change
#African Elephant
samples <- det.out.wZT8.ele$BUGSoutput$sims.list
...
waic_results <- calculate_waic(det.out.wZT8.ele)
