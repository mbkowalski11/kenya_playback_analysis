...
det.out.MultiSpec <- R2jags::jags(data = data, inits = inits, parameters.to.save = parameters, 
                                  model.file = "MFP_MultiSpecies_SimpProc5.jags", n.chains = nc, 
                                  n.iter = ni, n.burnin = nb, n.thin = nthin)
...
desired <- c(
  ...
  "Spotted Hyena","Hares","Gunther's Dik-Dik","Leopard",
...
herbivores <- c("war.det", "lep.det", "gdd.det", "pze.det",
                "gvy.det", "imp.det", "gsq.det")
...
groups <- list(
  ...
  herbivores       = c("war","lep","gdd","pze","gvy","imp","gsq"),
...
save(trophic.group.out, file = "MFP_DetRate_Model6_TrophicGroups.rda")
