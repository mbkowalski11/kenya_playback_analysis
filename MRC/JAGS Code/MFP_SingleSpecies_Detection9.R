...
cat("model{
    ...
    ############
    # Create covariance matrices for spatial effects - EXPONENTIAL COVARIANCE
    for(j in 1:M){
        for(k in 1:M){
            # Exponential covariance function: sigma^2 * exp(-|d|/rho)
    ...
    # Derived parameters for spatial correlation assessment - EXPONENTIAL EFFECTIVE RANGE
    # For exponential covariance, effective range is typically rho * log(20) ≈ 3 * rho
    # This gives the distance at which correlation drops to ~0.05
    eff.range.psi <- rho.psi * 3
...
det.out.wZT9.ele <- jags(data = data, inits = inits, parameters.to.save = parameters,
...
# Save models
save(list = c("det.out.wZT9.ele"), file = 'MFP_DetRate_Model9ele.rda')

###################################################################################################
###################################################################################################
# Posterior Sampling of Bayesian P-Values and Percent Change
#African Elephant
samples <- det.out.wZT9.ele$BUGSoutput$sims.list
...
waic_results <- calculate_waic(det.out.wZT9.ele)
